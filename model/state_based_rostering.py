...
class Predict:
    ...
    def predict(
        ...
        shutdown_events = df_sales.loc[
            df_sales["Sales"] < 2000, "SpecialEvent"
        ].unique()
        ...
        min_train = 6 * 30  # at least 6 months of training data
        gap = 7 if n_samples > 4.0 * (min_train + horizon_days) else 0

        feasible = (n_samples - min_train - gap) // (horizon_days + gap)
        n_splits = max(3, min(10, int(feasible)))

        print(
            f"Using TimeSeriesSplit with n_splits={n_splits} test_size={horizon_days} and gap={gap}."
        )
        if gap > 0:
            tscv = TimeSeriesSplit(n_splits=n_splits, test_size=horizon_days, gap=gap)
        else:
